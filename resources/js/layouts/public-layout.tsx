...
export default function PublicLayout({ children }: PublicLayoutProps) {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100 selection:bg-orange-600 selection:text-white transition-colors duration-300">
            ...
            <header className="sticky top-0 z-40 w-full backdrop-blur-md bg-white/90 dark:bg-zinc-950/90 shadow-sm transition-colors duration-300">
                    ...
                    <nav className="hidden lg:flex items-center gap-8">
                        {navLinks.map((link) => (
                            <Link
                                key={link.label}
                                href={link.href}
                                className={`font-semibold text-sm transition-colors relative py-1 hover:text-orange-600 ${url === new URL(link.href).pathname
                            ...
                            <div className="flex flex-col gap-2">
                                {navLinks.map((link) => (
                                    <Link
                                        ...
                                        className={`block px-4 py-3 rounded-lg text-lg font-bold transition-all ${url === new URL(link.href).pathname
            ...
            <footer className="relative text-zinc-400 bg-zinc-950 dark:bg-black dark:text-zinc-400 pt-16 pb-8 transition-colors duration-300">
                {/* Background Image with Overlay */}
                <div className="absolute inset-0 bg-[url('/assets/images/splash1.jpg')] bg-no-repeat bg-cover bg-center opacity-10"></div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="flex gap-4">
                                {settings?.facebook_url && (
                                    <a href={settings.facebook_url} target="_blank" rel="noreferrer" className="p-2 bg-zinc-800 rounded-lg hover:bg-orange-600 hover:text-white transition-colors" aria-label="Facebook">
                                        <Facebook className="w-4 h-4" />
                                    </a>
                                )}
                                {settings?.twitter_url && (
                                    <a href={settings.twitter_url} target="_blank" rel="noreferrer" className="p-2 bg-zinc-800 rounded-lg hover:bg-orange-600 hover:text-white transition-colors" aria-label="Twitter">
                                        <Twitter className="w-4 h-4" />
                                    </a>
                                )}
                                {settings?.instagram_url && (
                                    <a href={settings.instagram_url} target="_blank" rel="noreferrer" className="p-2 bg-zinc-800 rounded-lg hover:bg-orange-600 hover:text-white transition-colors" aria-label="Instagram">
                                        <Instagram className="w-4 h-4" />
                                    </a>
                                )}
                                {settings?.linkedin_url && (
                                    <a href={settings.linkedin_url} target="_blank" rel="noreferrer" className="p-2 bg-zinc-800 rounded-lg hover:bg-orange-600 hover:text-white transition-colors" aria-label="LinkedIn">
